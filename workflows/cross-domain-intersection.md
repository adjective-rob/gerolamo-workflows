...
you_will_get:
  ...
  - Top 3 sources from each selected domain
  - New sources submitted from intersection research
  - 2 concepts (one cross-domain concept, one concrete implementation)
...
3. Search the Gerolamo corpus for sources in each of the domains you identified. Use `query_intelligence` for each domain. Show the top 3 results from each.
...
7. Use `compose_molecules` to create a 'build' composition combining the real sources from step 3 and both concepts. Add `user_context`: 'Design this as a production-ready open source project with a clear README, installation guide, and API surface. Focus on the novel cross-domain aspects that make this different from existing tools in either domain.'
